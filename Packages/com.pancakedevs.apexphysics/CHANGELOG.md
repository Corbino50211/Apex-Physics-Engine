...
- **Fracture Trigger** option on `ApexDestructible` with **Manual Only** and **Impact Threshold** modes.
- **Threshold Measurement** option with **Collision Impulse** and **Estimated Force** modes.
- Estimated-force thresholds in newtons, calculated from Unity collision impulse divided by the current Fixed Timestep.
- Inspector guidance that separates editor-time geometry generation from runtime fracture activation.

### Changed

- Existing `Break Impulse` values migrate automatically to the new **Break Threshold** field.
- Automatic impact fracture remains the default behavior for newly generated destructibles.
- `BreakAt`, `BreakAll`, UnityEvents, and Play Mode test buttons work regardless of the selected trigger mode.
- Debris momentum continues to use the original collision impulse even when Estimated Force is selected for threshold comparison.

### Notes

- This update does not perform mesh slicing during gameplay. It activates the chunks generated by the editor fracture workflow.
- Estimated Force is an approximation because Unity resolves collision impulses over physics steps.
- Unity compilation and Play Mode threshold behavior still require confirmation in the Unity 6.2 Windows test project.
